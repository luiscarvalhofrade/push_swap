...
/*   Updated: 2025/01/30 16:54:50 by luide-ca         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (current2)
	{
		...
		while (current1)
		{
			if (current1->index > current2->index && \
				current1->index < best_match->index)
				best_match = current1;
...
void	cal_cost_b(t_elem **lst1, t_elem **lst2)
{
	...
	while (current2)
	{
		if (current2->position <= (ft_lstsize(*lst2) - 1) / 2
			&& current2->target_pos->position <= (ft_lstsize(*lst1) - 1) / 2)
			current2->cost = ft_greater(current2->position, current2->target_pos->position);
		else if (current2->position > (ft_lstsize(*lst2) - 1) / 2
			&& current2->target_pos->position > (ft_lstsize(*lst1) - 1) / 2)
			current2->cost = ft_greater(ft_lstsize(*lst2) - current2->position,
					ft_lstsize(*lst1) - current2->target_pos->position);
		else if (current2->position <= (ft_lstsize(*lst2) - 1) / 2
			&& current2->target_pos->position > (ft_lstsize(*lst1) - 1) / 2)
			current2->cost = current2->position + ft_lstsize(*lst1)
				- current2->target_pos->position;
		else
